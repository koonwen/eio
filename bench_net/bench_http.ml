...
let metric_create name time value units description : Yojson.Safe.t =
  `Assoc [
    "name", `String name;
    "time", (time :> Yojson.Safe.t);
    "units", `String "ms";
...
let main net domain_mgr ~n_client_domains ~n_server_domains ~n_connections_per_domain ~n_requests_per_connection =
  ...
  let time = t1 -. t0 in
  let req_per_s = float requests /. time in
  metric_create
    (Printf.sprintf "requests:%d client-domains:%d server-domains:%d" requests n_client_domains n_server_domains)
   (`Float (time *. 1000.)) (`Float req_per_s) "requests/s" "Request rate of a HTTP client/server system"
